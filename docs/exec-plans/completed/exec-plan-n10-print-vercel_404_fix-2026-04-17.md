...
Vercel 대시보드에서 프로젝트 설정을 수정하여 서비스 경로를 명시하고 필요한 환경 변수를 등록합니다.
...
#### API Key (Environment Variables) 등록
1. **Settings** -> **Environment Variables** 탭으로 이동합니다.
2. 로컬의 `.env.local` 파일에 있는 변수들을 등록합니다.
   - **Key**: `GOOGLE_AI_API_KEY`
   - **Value**: (사용자의 구글 AI API 키 입력)
3. **Add**를 클릭하여 저장합니다.

#### 재배포 (Redeploy)
1. 모든 설정이 완료되면 **Deployments** 탭에서 최신 커밋을 다시 배포(Redeploy)합니다.
