...
export default function LibraryModal({
  ...
  const showCheckboxes = isDeleteMode || isMultiSelect
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files && files.length > 0 && activeFolderId && onAddImageToFolder) {
      Array.from(files).forEach(f => onAddImageToFolder(activeFolderId, f))
    }
        ...
        <div className="flex-1 overflow-y-auto p-5">
          {folders.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-gray-400">
              ...
              <p className="mt-2 text-sm">폴더가 없습니다</p>
            </div>
          ) : !activeFolder ? (
            /* 폴더 목록 */
