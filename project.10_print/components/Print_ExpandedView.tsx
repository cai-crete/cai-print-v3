...
/**
 * Print_ExpandedView.tsx — N10 print 노드 (Controlled 컴포넌트)
 *
 * 역할: 전체 앱 상태를 중앙에서 관리하고, 각 컴포넌트에 필요한 props만 주입한다.
 * Canvas 앱에 이식하기 위해 props 로직 (selectedImages, onSave 등)이 추가되었다.
 *
...
export function PrintExpandedView(props: PrintExpandedViewProps) {
    ...
    const processImages = async () => {
      try {
        ...
        if (!libraryActionTarget || libraryActionTarget === 'common') {
          ...
          setVideoStartImage(newFiles[0] ?? null);
        } else if (libraryActionTarget === 'videoEnd') {
          setVideoEndImage(newFiles[0] ?? null);
        }
      } catch (err) {
        console.error('Failed to parse selected images', err);
      }
    };
  ...
  useEffect(() => {
    if (props.initialAction === 'video') {
      setMode('VIDEO')
    } else if (props.initialAction === 'generate') {
      // 아무것도 하지 않음 (기본 화면)
    }
  }, [props.initialAction]);
  ...
  }, [libraryFolders.length, isLibraryLoading, apiBaseUrl])

  useEffect(() => {
    if (props.initialAction === 'library') {
      handleOpenLibrary()
    }
  }, [props.initialAction, handleOpenLibrary])
  ...
  }, [apiBaseUrl, mode, images, videoStartImage, videoEndImage, prompt, pageCount, historyIndex])
  ...
  return (
    <div data-print-canvas className={props.className || ''}>
      ...
      {/* 5. 우측 사이드바 (Canvas Wrapper 우선, 없으면 Standalone Sidebar) */}
      {props.renderSidebarWrapper ? (
        props.renderSidebarWrapper(
          <div className="flex flex-col h-full">
            <div className="flex flex-col gap-8 flex-1">
              ...
              {agentError && (
                <div style={{
                  border: '1px solid #f87171', borderRadius: '0.5rem', padding: '0.75rem',
                  backgroundColor: '#fff1f2', fontSize: '0.75rem', lineHeight: '1.5', color: '#b91c1c'
                }}>
                  <p style={{ fontWeight: 600, marginBottom: '0.25rem' }}>✕ {agentError.failedAgent} 단계 실패</p>
                  <p style={{ color: '#6b7280', marginBottom: '0.375rem' }}>유형: {agentError.errorType}</p>
                  <p style={{ marginBottom: '0.5rem' }}>{agentError.message}</p>
                  <button onClick={() => setAgentError(null)} style={{ marginTop: '0.5rem', fontSize: '0.7rem', color: '#9ca3af', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>닫기</button>
                </div>
              )}
            </div>
